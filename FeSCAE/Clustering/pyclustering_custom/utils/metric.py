...
class type_metric(IntEnum):
    ...
    ## Cosine distance, for more information see function 'cosine_distance'.
    COSINE = 8

    ## Pearson correlation distance, for more information see function 'pearson_distance'.
    PEARSON = 9
    
    ## Spearman correlation distance, for more information see function 'spearman_distance'.
    SPEARMAN = 10
...
class distance_metric:
    ...
    def __create_distance_calculator_basic(self):
        ...
        elif self.__type == type_metric.COSINE:
            return cosine_distance
        
        elif self.__type == type_metric.PEARSON:
            return pearson_distance
        
        elif self.__type == type_metric.SPEARMAN:
    ...
    def __create_distance_calculator_numpy(self):
        ...
        if self.__type == type_metric.EUCLIDEAN:
            ...
            return cosine_distance_numpy
        
        elif self.__type == type_metric.PEARSON:
            return pearson_distance_numpy
        
        elif self.__type == type_metric.SPEARMAN:
            return spearman_distance_numpy
...
def cosine_distance(point1, point2):
    ...
    @details Cosine distance is calculated using following formula:
    \f[
    dist(a, b) = 1 - \frac{a \cdot b}{\left \| a \right \| \left \| b \right \|};
    \f]

    @param[in] point1 (array_like): The first vector.
    @param[in] point2 (array_like): The second vector.

    @return (float) Cosine distance between two objects.
    ...
    return 1.0 - abs((dot_product / ((norm_a ** 0.5) * (norm_b ** 0.5))))

def cosine_distance_numpy(object1, object2):
    """!
    @brief Calculate cosine distance between two vectors using numpy.
    ...
    """
    dot_product = numpy.sum(object1 * object2, axis=-1)
    norm_a = numpy.linalg.norm(object1, axis=-1)
    norm_b = numpy.linalg.norm(object2, axis=-1)

    # Avoid division by zero
    norm_a[norm_a == 0] = 1
    norm_b[norm_b == 0] = 1

    return 1.0 - numpy.abs(dot_product / (norm_a * norm_b))

def pearson_distance(point1, point2):
    """!
    @brief Calculate Pearson correlation distance between two vectors.
    @details Pearson distance is calculated as:
    \f[
    dist(a, b) = 1 - \rho_{pearson}(a, b)
    \f]

    where \f$\rho_{pearson}(a, b)\f$ is the Pearson correlation coefficient.

    @param[in] point1 (array_like): The first vector.
    @param[in] point2 (array_like): The second vector.

    @return (float) Pearson correlation distance between two objects.
    """
    mean_a = sum(point1) / len(point1)
    mean_b = sum(point2) / len(point2)
    ...
    return 1.0 - abs((num / ((denom_a ** 0.5) * (denom_b ** 0.5))))

def pearson_distance_numpy(object1, object2):
    """!
    @brief Calculate Pearson correlation distance between two vectors using numpy.

    @param[in] object1 (array_like): The first vector or matrix.
    @param[in] object2 (array_like): The second vector or matrix.

    @return (float or ndarray): Pearson distance between the two objects.
    """
    mean1 = numpy.mean(object1, axis=-1, keepdims=True)
    mean2 = numpy.mean(object2, axis=-1, keepdims=True)

    da = object1 - mean1
    db = object2 - mean2

    num = numpy.sum(da * db, axis=-1)
    denom_a = numpy.sqrt(numpy.sum(da ** 2, axis=-1))
    denom_b = numpy.sqrt(numpy.sum(db ** 2, axis=-1))

    # Avoid division by zero
    denom_a = numpy.where(denom_a == 0, 1, denom_a)
    denom_b = numpy.where(denom_b == 0, 1, denom_b)

    return 1.0 - numpy.abs((num / (denom_a * denom_b)))

def spearman_distance(point1, point2):
    """!
    @brief Calculate Spearman correlation distance between two vectors.
    @details Spearman distance is calculated as:
    \f[
    dist(a, b) = 1 - \rho_{spearman}(a, b)
    \f]

    where \f$\rho_{spearman}(a, b)\f$ is the Spearman rank correlation coefficient.

    @param[in] point1 (array_like): The first vector.
    @param[in] point2 (array_like): The second vector.

    @return (float) Spearman correlation distance between two objects.
    """
    from scipy.stats import rankdata

    rank_a = rankdata(point1)
    rank_b = rankdata(point2)
    ...
    return 1.0 - (num / ((denom_a ** 0.5) * (denom_b ** 0.5)))

def spearman_distance_numpy(object1, object2):
    """!
    @brief Calculate Spearman correlation distance between two vectors using numpy.

    @param[in] object1 (array_like): The first vector or matrix.
    @param[in] object2 (array_like): The second vector or matrix.

    @return (float or ndarray): Spearman distance between the two objects.
    """
    from scipy.stats import rankdata

    if len(object1.shape) == 1:
        rank1 = rankdata(object1)
        rank2 = rankdata(object2)
    else:
        rank1 = numpy.apply_along_axis(rankdata, 1, object1)
        rank2 = numpy.apply_along_axis(rankdata, 1, object2)

    mean1 = numpy.mean(rank1, axis=-1, keepdims=True)
    mean2 = numpy.mean(rank2, axis=-1, keepdims=True)

    da = rank1 - mean1
    db = rank2 - mean2

    num = numpy.sum(da * db, axis=-1)
    denom1 = numpy.sqrt(numpy.sum(da ** 2, axis=-1))
    denom2 = numpy.sqrt(numpy.sum(db ** 2, axis=-1))

    # Avoid division by zero
    denom1 = numpy.where(denom1 == 0, 1, denom1)
    denom2 = numpy.where(denom2 == 0, 1, denom2)

    return 1.0 - (num / (denom1 * denom2))