from ..pyclustering_custom.cluster.kmedoids import kmedoids
from ..pyclustering_custom.utils.metric import distance_metric, type_metric
from sklearn.preprocessing import StandardScaler
from .tools import ClusteringLogger, evaluate_cluster, select_best_clustering
import pandas as pd
...
def group_features_by_kmedoids(df, params_kmedoids, mode='searching', output_dir='dataset_training'):
    ...
    metric_type = params_kmedoids['metric_type']
    
    if metric_type == 'euclidean':
        scaler = StandardScaler()
        df = pd.DataFrame(scaler.fit_transform(df), columns=df.columns)

    logger = ClusteringLogger(log_filename=f"kmedoids_clustering_{metric_type}.log", log_dir=os.path.join(output_dir, 'clustering_searh_logs'))
    ...
    # Esegui il clustering KMedoids
    metric = distance_metric(getattr(type_metric, metric_type.upper()))
    
    # Parametri per l'early stopping
    patience = 5  # Numero di iterazioni consecutive con lo stesso miglior risultato
    best_result = None
    patience_counter = 0

    for num_clusters in range(min_clusters, max_clusters + 1):
        # Inizializza i medoidi
        # Usiamo random.sample con random_state per riproducibilità
        random.seed(42)
        initial_medoids = random.sample(range(num_features), num_clusters)
        
        kmedoids_instance = kmedoids(df.T.values.tolist(), initial_medoids, metric=metric, data_type='points')
    ...
    if mode == 'searching':
        return {
            'params': params_kmedoids,
            'best_result': best_result
        }
    
    final_logger = ClusteringLogger(log_filename=f"kmedoids_clustering_best.log", log_dir=output_dir)
    ...
    random.seed(42)
    initial_medoids = random.sample(range(num_features), n_clusters)
    kmedoids_instance = kmedoids(df.T.values.tolist(), initial_medoids, metric=metric, data_type='points')
    kmedoids_instance.process()
    clusters = kmedoids_instance.get_clusters()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'feature_counts_per_cluster_kmedoids.png'))

    return final_df


def params_search_kmedoids(df, output_dir='dataset_training'):
    """Esegue la ricerca dei migliori parametri per KMedoids"""
    
    metric_types = ['euclidean', 'cosine', 'pearson', 'spearman']
    
    logger = ClusteringLogger(log_filename="kmedoids_clustering_search.log", log_dir=os.path.join(output_dir,'clustering_searh_logs'))
    ...
    jobs = [delayed(group_features_by_kmedoids)(df, p, mode='searching', output_dir=output_dir) for p in params]
    results = Parallel(n_jobs=-1, verbose=1)(jobs)
    
    for result in results:
        logger.log(result)
    
    # Trova la combinazione di parametri con il miglior risultato
