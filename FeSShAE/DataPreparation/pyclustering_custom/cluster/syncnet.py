...
from pyclustering_custom.cluster.encoder import type_encoding
from pyclustering_custom.cluster import cluster_visualizer

from pyclustering_custom.core.syncnet_wrapper import syncnet_create_network, syncnet_process, syncnet_destroy_network, syncnet_analyser_destroy
from pyclustering_custom.core.sync_wrapper import sync_connectivity_matrix
from pyclustering_custom.core.wrapper import ccore_library

from pyclustering_custom.nnet.sync import sync_dynamic, sync_network, sync_visualizer
from pyclustering_custom.nnet import conn_represent, initial_type, conn_type, solve_type

from pyclustering_custom.utils import euclidean_distance
...
class syncnet_analyser(sync_dynamic):
    ...
    def allocate_clusters(self, eps = 0.01, indexes = None, iteration = None):
        """!
        @brief Returns list of clusters in line with state of oscillators (phases).
        
        @param[in] eps (double): Tolerance that defines the maximum difference between phases of oscillators that belong to one cluster.
